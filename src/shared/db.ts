...
export async function getAllProjects(): Promise<Project[]> {
  ...
  return (result.Items || []).map(({ PK, SK, ...project }) => project as Project);
}
...
export async function getProjectIdsForClaim(claimId: string): Promise<string[]> {
  const result = await docClient.send(
    new QueryCommand({
      TableName: TABLE_NAMES.CLAIM_PROJECTS,
...
export async function getClaimIdsForProject(projectId: string): Promise<string[]> {
  const result = await docClient.send(
    new QueryCommand({
      TableName: TABLE_NAMES.CLAIM_PROJECTS,
...
export async function deleteAllProjectLinksForClaim(claimId: string): Promise<void> {
  const projectIds = await getProjectIdsForClaim(claimId);
...
export async function deleteAllClaimLinksForProject(projectId: string): Promise<void> {
  const claimIds = await getClaimIdsForProject(projectId);
